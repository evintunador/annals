...
import { incomingRef, notesRef, type Ledger } from "./ledger.js";
import {
  finalizeEvent,
  parseEventLine,
  serializeEvent,
...
export { ensureMergeConfig } from "./transport.js";
...
function stateDir(repo: Ledger): string {
  return join(repo.commonDir, repo.ns.stateDirName);
}

function pendingPath(repo: Ledger): string {
  return join(stateDir(repo), "pending.jsonl");
}

/** Local, rebuildable cache/state only — never the record of truth. */
async function ensureStateDir(repo: Ledger): Promise<void> {
  await mkdir(stateDir(repo), { recursive: true });
}

export async function captureContext(repo: Ledger): Promise<RepoContext> {
...
async function readNoteLines(repo: Ledger, anchor: string, refName = repo.ns.name): Promise<string[]> {
...
export async function listAnchors(repo: Ledger, refName = repo.ns.name): Promise<string[]> {
...
async function withLock<T>(repo: Ledger, fn: () => Promise<T>): Promise<T> {
...
export async function appendEvents(
  ...
  return withLock(repo, async () => {
    ...
    await git(["notes", "--ref", repo.ns.name, "add", "-f", "-F", "-", anchor], {
      cwd: repo.root,
      input: lines.join("\n") + "\n",
    });
...
async function appendPending(repo: Ledger, events: EvidenceEvent[]): Promise<AppendResult> {
  await ensureStateDir(repo);
...
async function drainPending(repo: Ledger): Promise<EvidenceEvent[]> {
  const path = pendingPath(repo);
...
export async function readPending(repo: Ledger): Promise<EvidenceEvent[]> {
  const path = pendingPath(repo);
  if (!existsSync(path)) return [];
...
export interface ReadOptions {
   ...
   * is checked out — maintenance passes (scan, review, inspect, renormalize) — and
   * they say so explicitly at the call site.
   */
  reachableFrom?: string | null;
  /** Exact match, or any-of when an array. Kinds are owned by
   * (producer.tool, kind); pair with `tool` to address one producer's
   * vocabulary. */
  kind?: string | string[];
...
export async function readEvents(repo: Ledger, opts: ReadOptions = {}): Promise<EvidenceEvent[]> {
  ...
  const kinds =
    opts.kind === undefined ? null : Array.isArray(opts.kind) ? new Set(opts.kind) : new Set([opts.kind]);
...
export async function manualReAnchor(
  ...
  const resolve = async (rev: string): Promise<string> => {
    ...
    if (!sha) throw new Error(`${repo.ns.cliName} re-anchor: cannot resolve "${rev}" to a commit`);
    return sha;
  };
 ...
 * `git fetch` + a read is enough for a squash-merged branch's
 ...
 * target was seen won't auto-map until the target moves again — `re-anchor` covers that manually.
 */
async function autoReAnchor(repo: Ledger): Promise<void> {
  try {
    const config = await loadConfig(repo);
...
export function sortEvents(events: EvidenceEvent[]): EvidenceEvent[] {
  return [...events].sort((a, b) => {
    const conv = (a.stream?.id ?? "").localeCompare(b.stream?.id ?? "");
    if (conv !== 0) return conv;
    const seq = (a.stream?.seq ?? 0) - (b.stream?.seq ?? 0);
    if (seq !== 0) return seq;
 ...
 * point at behind.
 */
async function anchorsForRevs(repo: Ledger, revs: string[]): Promise<string[]> {
...
async function pushScopedNotes(
  ...
  const tmpName = `${repo.ns.name}-push-tmp`;
  ...
  try {
    ...
    await git(["push", remote, `${tmpRef}:${notesRef(repo.ns)}`], { cwd: repo.root });
  } finally {
    await git(["update-ref", "-d", tmpRef], { cwd: repo.root, allowFailure: true });
  }
}
 ...
 */
async function remoteNoteIds(repo: Ledger, remote: string): Promise<Set<string> | null> {
  const remoteHasRef = (await git(["ls-remote", remote, notesRef(repo.ns)], {
    cwd: repo.root,
    allowFailure: true,
  })).trim();
...
/**
 * Thrown when redact is asked to rewrite an event whose
...
async function remoteLedgerState(repo: Ledger, remote: string): Promise<RemoteLedgerState> {
  ...
  try {
    const out = (await git(["ls-remote", remote, notesRef(repo.ns)], { cwd: repo.root })).trim();
...
/**
 * Sync of the namespace ref: explicit via the CLI sync command, or run by the
 * pre-push transport hook (see transportPush). The fetch phase stages into
 * incomingRef(repo.ns) and absorbs it — the same path a plain `git fetch` plus a
 * later read takes.
 */
export async function sync(
  ...
  if (mode !== "push") {
    await git(
      ["fetch", remote, `+${notesRef(repo.ns)}:${incomingRef(repo.ns)}`],
...
function idMatchesPrefix(id: string, prefix: string): boolean {
  ...
  const hash = id.startsWith("ev2-") ? id.slice(4) : id;
  return hash.startsWith(prefix);
}

/** Find the one event matching `idPrefix` across every anchor and the pending queue. */
async function locateEvent(repo: Ledger, idPrefix: string): Promise<LocatedEvent> {
  ...
  if (matches.length === 0) {
    throw new Error(`${repo.ns.cliName} redact: no event matches id prefix "${idPrefix}"`);
  }
  if (matches.length > 1) {
    throw new Error(
      `${repo.ns.cliName} redact: id prefix "${idPrefix}" is ambiguous (${matches.length} matches: ` +
 ...
 */
async function assertNotYetShared(repo: Ledger, eventId: string): Promise<void> {
  ...
  if (state.kind === "unreachable") {
    throw new RedactAfterShareError(
      eventId,
      "remote-unreachable",
      `${repo.ns.cliName} redact: refusing to redact ${eventId.slice(0, 16)} — cannot reach origin to ` +
  ...
  throw new RedactAfterShareError(
    eventId,
    "already-pushed",
    `${repo.ns.cliName} redact: refusing to redact ${eventId.slice(0, 16)} — it is already on origin, so ` +
      ...
      `  keeps every prior version (recoverable in any clone with \`git log -p ${notesRef(repo.ns)}\`),\n` +
...
export async function redactEvent(
  ...
  if (hasPattern === hasAll) {
    throw new Error(`${repo.ns.cliName} redact: exactly one of --pattern or --all is required`);
  }
  ...
  const { located, rewritten, newRecords, secretValues } = await withLock(repo, async () => {
    ...
    if (hasPattern) {
      ...
      try {
        ...
        throw new Error(
          `${repo.ns.cliName} redact: invalid --pattern regex: ${err instanceof Error ? err.message : String(err)}`,
        );
      }
      ...
      if (records.length === 0) {
        throw new Error(`${repo.ns.cliName} redact: --pattern matched nothing in event ${original.id}`);
      }
    ...
    if (rewritten.id !== original.id) {
      ...
      throw new Error(`${repo.ns.cliName} redact: internal error — rewritten event id drifted from the original`);
    }
    ...
    if (located.anchor !== null) {
      newLines.sort();
      await git(["notes", "--ref", repo.ns.name, "add", "-f", "-F", "-", located.anchor], {
        cwd: repo.root,
        input: newLines.join("\n") + "\n",
      });
    } else {
  ...
  if (located.event.stream) redactionDraft.stream = located.event.stream;

  const appendResult = await appendEvents(repo, [redactionDraft]);
  const redactionEvent = appendResult.appended[0];
  if (!redactionEvent) {
    throw new Error(`${repo.ns.cliName} redact: failed to append companion redaction event`);
  }
  ...
  let squashed = false;
  if (located.anchor !== null) {
    const tree = (await git(["rev-parse", `${notesRef(repo.ns)}^{tree}`], { cwd: repo.root })).trim();
    const squashedCommit = (
      await git(["commit-tree", tree, "-m", `${repo.ns.name}: notes history squashed after redaction`], {
        cwd: repo.root,
      })
    ).trim();
    await git(["update-ref", notesRef(repo.ns), squashedCommit], { cwd: repo.root });
    squashed = true;
  }
