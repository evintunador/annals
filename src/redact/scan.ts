...
export interface Finding {
  ...
  path: string;
  /** Match span within that string, so inspect can find it again. */
  start: number;
  end: number;
}
    ...
    const visit = (value: string, path: string): string => {
      ...
      for (const rule of rules) {
        for (const m of value.matchAll(rule.pattern)) {
          ...
          findings.push({
            eventId: event.id,
            ...(event.stream ? { stream: event.stream.id } : {}),
 ...
 * `allow` could not durably clear, because each new report minted a
 * new event. Coordinates cannot do that: an event id and a JSON path match no
 * secret rule. Content lives behind `inspect`, which writes to a file.
 */
export function formatFinding(f: Finding): string {
  const where = `${f.path}@${f.start}`;
  return `${f.eventId.slice(0, 16)}  ${f.stream ?? "-"}  ${f.occurred_at}  ${f.rule}  ${where}  [${f.fingerprint}]`;
}
 ...
 * this to a file rather than a terminal — see `inspect`.
 */
...
function allowlistPath(repo: Ledger): string {
  return join(repo.commonDir, repo.ns.stateDirName, "allowlist.json");
}
 ...
 */
export async function loadAllowlist(repo: Ledger, config?: AnnalsConfig): Promise<Set<string>> {
  const [local, global] = await Promise.all([
    readAllowlistFile(allowlistPath(repo)),
    readAllowlistFile(globalAllowlistPath(repo)),
  ]);
...
export async function addToAllowlist(
  ...
  const path = scope === "global" ? globalAllowlistPath(repo) : allowlistPath(repo);
