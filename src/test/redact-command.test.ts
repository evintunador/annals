...
test("redactEvent: --pattern rewrites content+raw, preserves the id, appends a redacts companion, and squashes local history", async () => {
  ...
  try {
    ...
    const log = await git(["log", "--oneline", "refs/notes/annals"], { cwd: repo.root });
...
test("redactEvent: refuses outright once the event is on origin, leaving the ledger untouched", async () => {
  ...
  try {
    ...
    const notesBefore = (await git(["rev-parse", "refs/notes/annals"], { cwd: repo.root })).trim();
    ...
    // to prevent.
    const notesAfter = (await git(["rev-parse", "refs/notes/annals"], { cwd: repo.root })).trim();
...
test("readEvents: a redacted copy always beats its pre-redaction original, whatever the bytes say", async () => {
  ...
  try {
    ...
    await git(["notes", "--ref", "annals", "add", "-f", "-F", "-", anchor], {
      cwd: repo.root,
      input: lines.join("\n") + "\n",
    });
...
test("redactEvent: throws when no event matches the id prefix", async () => {
  ...
  try {
    await makeCommit(repo, "init");
    await assert.rejects(() => redactEvent(repo, "ev2-doesnotexist", { all: true }), /no event matches/);
  } finally {
    await cleanupRepo(repo);
  }
});
...
test("redactEvent: requires exactly one of --pattern or --all", async () => {
  ...
  try {
    await makeCommit(repo, "init");
    await assert.rejects(() => redactEvent(repo, "ev2-x", {}), /exactly one of/);
    await assert.rejects(
      () => redactEvent(repo, "ev2-x", { pattern: "x", all: true }),
      /exactly one of/,
    );
  } finally {
    await cleanupRepo(repo);
