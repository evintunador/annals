...
async function findRepoLedger(dir: string) {
  const r = await findRepo(dir);
  return r ? openLedger(r) : null;
}
...
async function makeIdentitylessRepo(): Promise<Ledger> {
  const dir = await mkdtemp(join(tmpdir(), "cledger-ident-"));
  await git(["init", "-q", "-b", "main"], { cwd: dir });
  const repo = await findRepoLedger(dir);
