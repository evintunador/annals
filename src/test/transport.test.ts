...
function hookPath(repo: Ledger): string {
...
test("ensureTransport: installs the pre-push hook and origin fetch refspec, idempotently", async () => {
  ...
  try {
    ...
    assert.ok(script.includes("annals pre-push"));
    ...
    assert.ok(fetchSpecs.includes(`+${"refs/notes/annals"}:${"refs/notes/annals-incoming"}`));
...
test("absorbIncoming: a staged fetch is folded into the local ref at read time", async () => {
  ...
  try {
    ...
    await git(["fetch", "origin", `+${"refs/notes/annals"}:${"refs/notes/annals-incoming"}`], { cwd: b.root });
    ...
    const incoming = (await git(["rev-parse", "--verify", "--quiet", "refs/notes/annals-incoming"], {
      cwd: b.root,
      allowFailure: true,
    })).trim();
