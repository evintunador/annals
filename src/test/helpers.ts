 ...
 *
 * loadConfig() reads user-level config at `~/.config/annals/config.json` via
 ...
 */
const ISOLATED_HOME = mkdtempSync(join(tmpdir(), "annals-test-home-"));
...
export async function makeTempRepo(prefix = "annals-test-"): Promise<Ledger> {
...
export async function makeBareRepo(prefix = "annals-remote-"): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), prefix));
