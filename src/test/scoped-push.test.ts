...
async function remoteEventIds(repo: Ledger, remote: string): Promise<Set<string>> {
  const tmp = "scoped-push-assert";
  const tmpRef = `refs/notes/${tmp}`;
  await git(["update-ref", "-d", tmpRef], { cwd: repo.root, allowFailure: true });
  await git(["fetch", remote, `+${"refs/notes/annals"}:${tmpRef}`], { cwd: repo.root, allowFailure: true });
...
async function record(repo: Ledger, text: string): Promise<string> {
...
test("an already-installed hook block is upgraded in place, preserving the rest of the file", async () => {
  ...
  try {
    ...
    const stale = [
      ...
      "# >>> annals pre-push (added by annals) >>>",
      "annals transport-push \"$1\" </dev/null || exit $?",
      "# <<< annals pre-push <<<",
    ...
    assert.ok(after.includes("_annals_refs"), "the current block reads the pushed refs");
  } finally {
    await cleanupRepo(repo);
  }
});
